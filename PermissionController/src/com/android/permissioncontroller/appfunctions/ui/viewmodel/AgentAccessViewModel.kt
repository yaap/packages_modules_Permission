...
class AgentAccessViewModel(
    ...
    private fun refresh() {
        coroutineScope.launch(dispatcher) {
            try {
                ...
                _uiStateFlow.value =
                    Stateful.Success(
                        createAgentAccessUiState(agentPackageName, accessRequestStates)
                    )
            } catch (e: Exception) {
    ...
    private fun createAgentAccessUiState(
        ...
        val allowedTargetPackageNames =
            accessRequestStates.filter { (_, isAllowed) -> isAllowed }.keys.toList()
        val notAllowedTargetPackageNames =
            accessRequestStates.filter { (_, isAllowed) -> !isAllowed }.keys.toList()
        return AgentAccessUiState(
            agentPackageName,
            allowedTargetPackageNames,
            notAllowedTargetPackageNames,
        )
    }
}
...
data class AgentAccessUiState(
    ...
    val allowedTargetPackageNames: List<String> = emptyList(),
    val notAllowedTargetPackageNames: List<String> = emptyList(),
)
